var app = angular.module('dsb', ['ng', 'ngRoute']);
//配置路由词典
...
app.controller('mainCtrl', ['$scope', '$http',
  function ($scope, $http) {
    ...
    $scope.loadMore = function () {
      $http
        .get('data/book_getbypage.php?start=' +
    ...
    $scope.$watch('kw', function () {
      if ($scope.kw.length > 0) {
        ...
        $http
          .get('data/book_getbykw.php?kw=' + $scope.kw)
          .success(function (data) {
            //console.log(data);
            if (data.length > 0) {
              $scope.bookList = data;
...
app.controller('detailCtrl', ['$scope', '$routeParams', '$http', function ($scope, $routeParams, $http) {
  ...
  $http
    .get('data/book_getbyid.php?did=' + $routeParams.did)
    .success(function (data) {
      //console.log(data);
      $scope.book = data[0];
    })
